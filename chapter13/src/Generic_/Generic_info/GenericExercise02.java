...
/**
 * @author hxz
 * 如果对某一个对象进行比较，下面是对 brithday 进行比较，放在 MyDate类中
 * 因此，需要对 brithday 进行比较时，最好就放在该类中
 * 封装后，将来可维护性和复用性，将大大增强
 *
 */
...
public class GenericExercise02 {
    public static void main(String[] args) {
        ...
        employees.add(new Employee02("hxz1", 35000, 18,
                new MyDate(2001, 4, 26)));
        ...
        employees.sort(new Comparator<Employee02>() {
            ...
            public int compare(Employee02 o1, Employee02 o2) {
                //定制排序
                //先对传入参数进行验证
                if (!(o1 instanceof Employee02 && o2 instanceof Employee02)){
                    System.out.println("类型不正确...");
                    return 0;
                }
                //按照姓名长度进行排序（从小到大）
//                int len = o1.getName().length() - o2.getName().length();
                //String 也有重写comparaTo 方法，长度一样按照 Ascall 码排序
                //都相同按照长度
                int len = o1.getName().compareTo(o2.getName());
                if (len > 0) {
                    return 1;
                } else if (len < 0) {
                    return -1;
